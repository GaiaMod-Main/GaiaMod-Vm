...
class Scratch3LearningMLImagesBlocks {
    ...
    getInfo() {
        return {
            ...
            blocks: [{
                ...
                //         description: 'Add an image to a label or class'
                //     }),
                //     arguments: {
                //         ITEM: {
                //             type: ArgumentType.STRING,
                ...
                // },
                // {
                //     opcode: 'learnFromImagesAndWait',
                //     blockType: BlockType.COMMAND,
                //     text: formatMessage({
                //         id: 'learningml.learnandwait',
                //         default: 'Learn from images dataset and wait',
                //         description: 'Learn from images dataset and wait'
                //     })
                // }   
            ],
            menus: {
    ...
    _classifyCostumeImage(operation, args, utils) {
        ...
        return lmlMessageProtocol.lmlRequest(operation, [src]);

    }

    _classifyVideoImage(operation) {
    ...
    classifyImage(args, utils) {
        ...
        if (args.ITEM == 'video') {
            return this._classifyVideoImage("classify_image");
        } else {
            return this._classifyCostumeImage("classify_image", args, utils);
        }
    }
    ...
    confidenceImage(args, utils) {
        ...
        if (args.ITEM == 'video') {
            return this._classifyVideoImage("confidence_image");
        } else {
            return this._classifyCostumeImage("confidence_image", args, utils);
        }
    }
    ...
    //     if (args.ITEM == 'video') {
    //         let video = this.runtime.ioDevices.video.provider._video;
    //         if(!video) return new Promise(resolve => {resolve("video is off")});
    ...
    //         return lmlMessageProtocol.lmlRequest('add_image_to_label', [src, args.LABEL])
    //     } else {
    //         let sprite = utils.target.sprite.costumes[args.ITEM - 1];
    //         let src = this.encodeDataURI(sprite.asset);
